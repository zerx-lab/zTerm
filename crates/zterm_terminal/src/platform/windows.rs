...
impl WindowsPty {
    ...
    pub fn new(config: PtyConfig, output_tx: Sender<Vec<u8>>) -> Result<Self> {
        ...
        let pair = pty_system
            .openpty(size)
            .map_err(|e| Error::pty(format!("Failed to open ConPTY: {}", e)))?;
        ...
        let writer = pair
            .master
            .take_writer()
            .map_err(|e| Error::pty(format!("Failed to get ConPTY writer: {}", e)))?;

        // Get reader and spawn reader thread
        let mut reader = pair
            .master
            .try_clone_reader()
            .map_err(|e| Error::pty(format!("Failed to get ConPTY reader: {}", e)))?;
    ...
    pub fn write(&self, data: &[u8]) -> Result<()> {
        ...
        writer
            .write_all(data)
            .map_err(|e| Error::pty(format!("Failed to write to ConPTY: {}", e)))?;
        writer
            .flush()
            .map_err(|e| Error::pty(format!("Failed to flush ConPTY: {}", e)))?;
        Ok(())
    }
    ...
    pub fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        pair.master
            .resize(size)
            .map_err(|e| Error::pty(format!("Failed to resize ConPTY: {}", e)))?;
        Ok(())
    }
}
